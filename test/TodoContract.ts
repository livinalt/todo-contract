...
describe("TodoContract", function () {
  ...
  describe("Create Todo", function () {
    it("Should create a todo instance", async function () {
      ...
      const create = await todoContract.createTodo("Jerry", "Staff Meeting","started");
      const todoList = await todoContract.getAllList() 

      expect(todoList.length).to.equal(1);
      expect(todoList[0].title).to.equal("Jerry");
      expect(todoList[0].description).to.equal("Staff Meeting");
      expect(todoList[0].status).to.equal("started");
    });
